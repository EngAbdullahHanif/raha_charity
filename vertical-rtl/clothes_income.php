...
if (isset($_POST['submit'])) {
    ...
    if ($name_err == "" and $quantity_err == "" and $unit_price_err == ""  and $type_err == "" and $date_err == "") {
        ...
        if ($result) {
            ...
            header('Location:clothes_income.php?msg=success');
        } else {
            //Data failed to insert
            header('Location:clothes_income.php?msg=faild');
        }
    } else {
        "Errors";
    }
...
if (isset($_GET['d-id'])) {
    ...
    if ($result) {
        header('Location:clothes_income.php?msg=delete_success');
    } else {
        header('Location:clothes_income.php?msg=delete_faild');
    }
}
                                ...
                                <div class="card text-white m-b-30 ">
                                    <div class="card-body special-card">
                                        ...
                                        <h4 class="mt-0 header-large "
                                            style="text-align: center; margin:15px; padding:10px;"> فرم ورودی لباس</h4>

                                        <form action="clothes_income.php" method="post" class="form-horizontal">
                                            <?php
                                ...
                                <div class="card-body primary-card">
                                    ...
                                    <table id="datatable" class="table table-bordered dt-responsive nowrap"
                                       dir="rtl" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                        ...
                                        <tbody>
                                            <?php
                                            $products = $child->select_registeration_product_clothes();
                                            $number = 1;
                                            ...
                                            <tr class="odd gradeX">
                                                ...
                                                <td><?php if ($product['type'] == 'purchased') {
                                                            echo 'خریده شده';
                                                        } elseif ($product['type'] == 'sales') {
                                                            echo 'فروخته شده';
                                                        } ?></td>
                                                ...
                                                <td>
                                                <?php echo$product['unit_price']*$product['quantity'] ?>
                                                </td>
                                                <td><?php echo $product['date'] ?></td>
                                                <td <?php if($_SESSION['type'] == 'admin'):  ?> class="hidden-phone">
                                                    <a href="#"><span class="btn btn-primary">ویرایش</span></a>
                                                    <a href="clothes_income.php?d-id=<?php echo $product['regis_id'] ?>"
                                    ...
                                    <?php
                                    $query = $child->select_all_clothes_price();
                                    $price = $query->fetch_assoc();
                                    ?>
                                    <h3> مجموعه قیمت های لباس = <?php echo $price['price'] ?></h3>
                                </div>
                            </div>
                        </div> <!-- end col -->
                    </div> <!-- end row -->
